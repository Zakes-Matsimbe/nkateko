...
const Assessments = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await api.get('/api/parent/assessments');
  ...
  return (
    <div className="py-4">
      <h2 className="mb-4 fw-bold">{childName}'s Assessments</h2>
                ...
                {rawAssessments.length > 0 ? (
                  rawAssessments.map((a, i) => (
                    <tr key={i}>
                      ...
                      <td>{new Date(a.date_written).toLocaleDateString('en-GB', { day : 'numeric', month: 'long', year: 'numeric' })}</td>
                      <td>{a.mark}%</td>
